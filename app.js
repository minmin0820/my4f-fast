...
function donut(id,obj,title){
 ...
 const entries=Object.entries(obj).map(([k,v])=>[k,Number(v)]).filter(([,v])=>Number.isFinite(v)&&v>0);
 for(const[k,v]of entries){const a1=a+v*3.6,mid=(a+a1)/2,[tx,ty]=polar(100,100,72,mid),c=colorFor(k);
 // SVG elliptical arcs cannot represent a full 360° circle when start=end.
 // Render a true circle for a single 100% holding so donut and legend use the exact same asset color.
 paths+=entries.length===1&&Math.abs(v-100)<1e-6
   ?`<circle cx="100" cy="100" r="72" fill="none" stroke="${c}" style="stroke:${c}" stroke-width="34"/>`
   :`<path d="${arc(100,100,72,a,a1)}" fill="none" stroke="${c}" style="stroke:${c}" stroke-width="34"/>`;
...
fetch(`kirin_snapshot.json?v=20260923-v158`,{cache:'no-store'}).then(r=>{if(!r.ok)throw Error(`snapshot ${r.status}`);return r.json()}).then(d=>{
 ...
 const note=document.querySelector('.dash-allocation-note');
 const fine=document.querySelector('.dash-fineprint');
 const renderDashPortfolio=name=>{
   ...
   const isPractical=/（現世）|「現世」/.test(name);
   if(note) note.textContent=isPractical?'R289 Practical / Canonical Position':'Canonical Position / Display only';
    ...
    <div class="mt-table">
      ...
      ${rows.map(r=>{
        ...
        return `<div class="mt-tr">
          <div class="mt-month">${monthLabel(r.month)}${r.mtd?'<span class="mt-mtd-badge">MTD</span>':''}${r.overlay?`<span class="mt-overlay-badge ${/反攻/i.test(r.overlay)?'is-hanko':/^A-STATE/i.test(r.overlay)?'is-astate':/^PROMOTION100/i.test(r.overlay)?'is-promotion':/^BOOSTER/i.test(r.overlay)?'is-booster':'is-other'}">${E(r.overlay)}</span>`:''}</div>
...
// v120 — Deterioration Monitor: display-only reader of Research/Python canonical Strategy Health.
(function(){
 function initDeteriorationPicker(){
  ...
  const hs=window.__MY4F_SNAPSHOT__?.strategy_health, data=hs?.portfolios||{};
  const allNames=Object.keys(data);
  const E=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const color=v=>String(v||'NEUTRAL').toUpperCase();
  const dot=v=>color(v)==='ORANGE'?'🟠':color(v)==='YELLOW'?'🟡':color(v)==='GREEN'?'🟢':'⚪';
  const cls=v=>color(v)==='GREEN'?'good':color(v)==='YELLOW'?'watch':color(v)==='ORANGE'?'orange':'na';
  const status=d=>{if(!d||d.audit_status!=='CALCULATED')return ['INSUFFICIENT DATA','na'];const cc=[d.g1?.color,d.g2?.color,d.p?.color,d.pbar?.color].map(color);if(cc.includes('ORANGE'))return ['DETERIORATING','bad'];if(cc.includes('YELLOW'))return ['MIXED','mixed'];return ['GOOD','good']};
  const asof=root.querySelector('.det-asof'); if(asof)asof.textContent=hs?`Research/Python canonical ｜ as of ${hs.as_of||'—'} ｜ ${hs.methodology||'Strategy Health'}`:'Strategy Health snapshot unavailable — FAIL-CLOSED';
  [...tbody.querySelectorAll('tr')].forEach(x=>x.remove());
  allNames.forEach(n=>{const d=data[n],st=status(d);tbody.insertAdjacentHTML('beforeend',`<tr data-det-generated="1" data-det-name="${E(n)}" data-det-status="${E(st[0])}" tabindex="0"><td>${E(n)}</td><td><i class="det-dot ${cls(d.g1?.color)}" title="${E(color(d.g1?.color))}"></i></td><td><i class="det-dot ${cls(d.g2?.color)}" title="${E(color(d.g2?.color))}"></i></td><td><i class="det-dot ${cls(d.p?.color)}" title="${E((d.p?.label||'INSUFFICIENT_DATA')+' / '+color(d.p?.color))}"></i></td><td><i class="det-dot ${cls(d.pbar?.color)}" title="${E((d.pbar?.label||'INSUFFICIENT')+' / '+color(d.pbar?.color))}"></i></td></tr>`)});
  const counts={bad:0,mixed:0,good:0,na:0}; allNames.forEach(n=>counts[status(data[n])[1]]++); const c=root.querySelector('.det-counts'); if(c)c.innerHTML=`<span><b>Deteriorating</b> <strong>${counts.bad}</strong></span><span><b>Mixed</b> <strong>${counts.mixed}</strong></span><span><b>Good</b> <strong>${counts.good}</strong></span><span><b>Insufficient</b> <strong>${counts.na}</strong></span>`;
  if(!allNames.length){mount.innerHTML='<div class="det-governance"><b>FAIL-CLOSED</b><span>Research/Python Strategy Health snapshotがありません。Fast側では再計算しません。</span></div>';return}
  const rows=[...tbody.querySelectorAll('tr')],prior=window.__MY4F_DET_SELECTED__,selected=new Set(prior instanceof Set?[...prior].filter(n=>allNames.includes(n)):allNames);window.__MY4F_DET_SELECTED__=selected;
  ...
  const wrap=mount.querySelector('[data-picker="deteriorationPicker"]'),panel=wrap.querySelector('.analytics-picker-panel'),tabs=wrap.querySelector('.analytics-picker-tabs'),label=wrap.querySelector('.analytics-selected-name'),count=wrap.querySelector('.det-selected-count'),groups=portfolioFoldGroups(allNames);let active='All';
  const healthInterpretation=d=>{
    if(d?.audit_status!=='CALCULATED') return '履歴不足のため4軸を統合評価できません。十分な履歴が蓄積するまでFAIL-CLOSEDで監視します。';
    const g1=color(d?.g1?.color),g2=color(d?.g2?.color),pc=color(d?.p?.color),bc=color(d?.pbar?.color);
    const pl=String(d?.p?.label||'').toUpperCase(), bl=String(d?.pbar?.label||'').toUpperCase();
    const structural=(g1==='ORANGE'?2:g1==='YELLOW'?1:0)+(g2==='ORANGE'?2:g2==='YELLOW'?1:0);
    const recent=(pc==='ORANGE'||pl==='DETERIORATING')?2:(pc==='YELLOW'||['WATCH','MIXED'].includes(pl))?1:0;
    const consistency=(bc==='GREEN'||bl==='STRONG')?2:(bc==='YELLOW'||bl==='MODERATE')?1:0;
    if(structural>=3 && recent>=2 && consistency===0) return '長期edgeの悪化方向・慢性的erosion・直近deteriorationが重なり、期間一貫性も弱い状態です。単なる短期不調より構造劣化仮説を優先して再監査する局面です。Health単独ではProductionを変更しません。';
    if(structural>=3 && recent>=2) return 'G1/G2とPが同時に警戒域で、長期的なedge侵食と直近弱体化が一致しています。原因分解とForward OOSの重点監視を優先する局面です。p̄が保たれていても、Health単独でProductionは変更しません。';
    if(structural>=2 && recent===0) return '直近Pは保たれていますが、G1/G2側に長期edgeの悪化兆候があります。足元の成績だけで安心せず、慢性的erosionが継続・拡大するかを重点監視します。';
    if(structural===0 && recent>=2 && consistency>=1) return '長期構造と期間一貫性は概ね保たれていますが、直近6/12/24Mで弱体化が出ています。まずは「最近の不調」としてForward OOSを重点監視し、G1/G2へ波及するかを確認します。';
    if(structural===0 && recent===0 && consistency>=1) return '長期edge、慢性的erosion、直近deterioration、期間一貫性の各軸に大きな警戒の重なりはありません。通常監視を継続します。';
    if(consistency===0) return '期間分割をまたいだabsolute consistencyが弱く、他のHealth軸が良好でもedgeの時間的一貫性には注意が必要です。Forward OOSと期間別挙動を重点確認します。';
    return '一部の軸に注意信号がありますが、4軸は同じ種類の劣化を測っていません。単独の色で判断せず、長期傾向・慢性erosion・直近弱体化・期間一貫性の組み合わせとして継続監視します。';
  };
  const showDetail=name=>{const d=data[name],box=root.querySelector('#detDetail');if(!box)return;const st=status(d),ok=d?.audit_status==='CALCULATED';box.innerHTML=`<div class="det-detail-kicker">RESEARCH / PYTHON CANONICAL</div><div class="det-detail-head"><h3>${E(name)}</h3><span class="det-status ${E(st[1])}">${E(st[0])}</span></div><div class="det-axis-values"><span><b>G1</b><em>${dot(d?.g1?.color)}</em><strong>${E(color(d?.g1?.color))}</strong></span><span><b>G2</b><em>${dot(d?.g2?.color)}</em><strong>${E(color(d?.g2?.color))}</strong></span><span><b>P</b><em>${dot(d?.p?.color)}</em><strong>${E((d?.p?.label||'INSUFFICIENT_DATA')+' / '+color(d?.p?.color))}</strong></span><span><b>p̄</b><em>${dot(d?.pbar?.color)}</em><strong>${E((d?.pbar?.label||'INSUFFICIENT')+' / '+color(d?.pbar?.color))}</strong></span></div><div class="det-combined-interpretation"><b>実運用での統合解釈</b><span>${E(healthInterpretation(d))}</span></div><p class="det-canonical-note">${ok?`Python正本で計算済み。History ${Number(d.history_months||0)} months。Fast側では再計算していません。`:`History ${Number(d?.history_months||0)} months。Frozen Healthに必要な96か月未満のためFAIL-CLOSED。`}</p>`};
  const apply=()=>{rows.forEach(r=>{const n=r.dataset.detName;r.hidden=!selected.has(n);r.classList.remove('selected')});const n=selected.size;label.textContent=n===allNames.length?'All Portfolios':n===1?[...selected][0]:`${n} Portfolios`;count.textContent=`${n} selected`;if(n===1)showDetail([...selected][0]);else{const box=root.querySelector('#detDetail');if(box)box.innerHTML=`<div class="det-detail-kicker">RESEARCH / PYTHON CANONICAL</div><h3>${n===0?'No Portfolio Selected':`${n} Portfolios Selected`}</h3><p>${n===0?'Selection Portfolioから1つ以上選択してください。':'Portfolio Tableで行をタップすると、Python正本の4軸詳細を表示します。'}</p>`}};
  ...
  rows.forEach(r=>{const pick=()=>{showDetail(r.dataset.detName);rows.forEach(x=>x.classList.toggle('selected',x===r))};r.onclick=pick;r.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();pick()}}});draw();apply();
 }
 window.__refreshDeteriorationPicker=initDeteriorationPicker;initDeteriorationPicker();
})();
