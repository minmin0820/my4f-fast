...
function renderMonthlyTrade(d){
 ...
 const asof=document.getElementById('mtAsOf');
 if(asof) asof.textContent=`as of ${d.asof||'—'}`;

 const current=d.execution||{};
 const month=d.month||'—';
 const prev=(d.action&&d.action.previous)||'—';
 const changed=!!(d.action&&d.action.changed);
 const added=(d.action&&d.action.added)||[];
 const removed=(d.action&&d.action.removed)||[];

 const ret=(d.returns||[]).find(r=>String(r[0]||'').startsWith(month));
 const mtd=ret&&Number.isFinite(Number(ret[3]))?Number(ret[3]):null;

 const entries=Object.entries(current).filter(([,v])=>Number(v)>0)
   .sort((a,b)=>Number(b[1])-Number(a[1]));
 const total=entries.reduce((s,[,v])=>s+Number(v),0);

 const actionText=changed
   ? `${added.length?`追加：${added.join(' / ')}`:''}${added.length&&removed.length?'　':''}${removed.length?`除外：${removed.join(' / ')}`:''}`
   : '保有アセット変更なし — 配分のみリバランス';

 root.innerHTML=`
   <div class="mt-status ${changed?'changed':''}">
     <span class="mt-status-dot"></span>
     <strong>${mtEsc(actionText)}</strong>
   </div>

   <div class="mt-ticket">
     <div class="mt-ticket-head">
       <div><span class="mt-label">TRADE MONTH</span><strong>${mtEsc(month)}</strong></div>
       <div class="mt-mtd ${mtd!=null&&mtd<0?'neg':''}">
         <span>MTD</span><strong>${mtd==null?'—':mtPct(mtd)}</strong>
       </div>
     </div>
     <div class="mt-holdings">
       ${entries.map(([k,v])=>`<div class="mt-holding-row"><span>${mtEsc(k)}</span><strong>${Number(v).toFixed(1)}%</strong></div>`).join('')}
     </div>
     <div class="mt-total"><span>Total</span><strong>${total.toFixed(1)}%</strong></div>
   </div>

   <div class="mt-change">
     <div class="mt-label">PREVIOUS MONTH</div>
     <div class="mt-prev-value">${mtEsc(prev)}</div>
   </div>

   <div class="mt-guide">
     <strong>Execution Guide</strong>
     <span>月初に上記比率へリバランス。Fast側では売買判定・配分計算を行わず、Python正本のsnapshotを表示します。</span>
   </div>
 `;
}
