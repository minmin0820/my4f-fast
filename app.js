...
(function(){
 function initDeteriorationPicker(){
  ...
  const showDetail=name=>{const d=data[name],box=root.querySelector('#detDetail');if(!box)return;const st=status(d),ok=d?.audit_status==='CALCULATED';box.innerHTML=`<div class="det-detail-kicker">RESEARCH / PYTHON CANONICAL</div><div class="det-detail-head"><h3>${E(name)}</h3><span class="det-status ${E(st[1])}">${E(st[0])}</span></div><div class="det-axis-values"><span><b>G1</b><em>${dot(d?.g1?.color)}</em><strong>${E(color(d?.g1?.color))}</strong></span><span><b>G2</b><em>${dot(d?.g2?.color)}</em><strong>${E(color(d?.g2?.color))}</strong></span><span><b>P</b><em>${dot(d?.p?.color)}</em><strong>${E((d?.p?.label||'INSUFFICIENT_DATA')+' / '+color(d?.p?.color))}</strong></span><span><b>p̄</b><em>${dot(d?.pbar?.color)}</em><strong>${E((d?.pbar?.label||'INSUFFICIENT')+' / '+color(d?.pbar?.color))}</strong></span></div><p>${ok?`Python正本で計算済み。History ${Number(d.history_months||0)} months。Fast側では再計算していません。`:`History ${Number(d?.history_months||0)} months。Frozen Healthに必要な96か月未満のためFAIL-CLOSED。`}</p>`};
