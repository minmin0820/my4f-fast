...
fetch(`kirin_snapshot.json?v=20260922-v144`,{cache:'no-store'}).then(r=>{if(!r.ok)throw Error(`snapshot ${r.status}`);return r.json()}).then(d=>{
 if(d.schema_version!=='kirin-fast-1.0')throw Error('unsupported snapshot schema');
 const FAST_BUILD_VERSION='v144';
 const FAST_BUILD_LABEL='Fast-owned Version';
 const fbEl=document.getElementById('fastBuildBadge');
 if(fbEl) fbEl.textContent=`✓ ${FAST_BUILD_VERSION} ${FAST_BUILD_LABEL}`;
